...
let cobra = [];
let direcao = {x: 1, y:0}
let proximaDirecao = {x: 1, y: 0}//Direção da próxima tickada(direção)
let comida = {x: 0, y: 0}
let pontuacao = 0;
let recorde = parseInt(localStorage.getItem("recorde_cobra"))|| 0;
let intervalo = null; //referencia de loop infinito do jogo
let emJogo = false;
...
function renderizar(){
    ...
    for (let i = 1; i < cobra.length; i++){
        celulas[cobra[i].y][cobra[i].x].classList.add
        ("cobra");
    }
    celulas[cobra[0].y][cobra[0].x].classList.add
    ("cabeca");
}
...
function encerrarJogo() {
    ...
    document.getElementsById("overlay-pontos").textContent = pontuacao + " pontos";
    document.getElementsById("overlay-recorde").textContent = novoRecorde
    ? " Novo   Recorde!"
    : "Recorde: " + recorde;
